...
class TestConnector:
    ...
    def test_list_all_connectors(self):
        ...
        assert recv == self.conn.ListAllConnectors('dev')

    def test_get_info_connector(self):
        recv = {
        ...
        result = self.conn.GetInfoConnector('dev', 'logs-broker')
        assert result == recv
    ...
    def test_get_connector_config(self):
        ...
        result = self.conn.GetConnectorConfig('dev', 'logs-broker')

        assert result == recv

    def test_get_connector_status(self):
        assert 'logs-broker' in self.conn.GetConnectorStatus('dev', 'logs-broker')['name']

    def test_get_connector_tasks(self):
        recv = [{'config': {
            'batch.size': '2000',
            'file': '/var/log/broker.log',
        ...
        result = self.conn.GetConnectorTasks('dev', 'logs-broker')
        assert result == recv

    def test_get_status_task(self):
        assert 'RUNNING' in self.conn.GetStatusTask('dev', 'logs-broker', '0')['state']

    def test_restart_connector_task(self):
        self.conn.RestartConnector('dev', 'logs-broker')

    def test_get_connector_plugins(self):
        recv = {
        ...
        result = self.conn.GetConnectorPlugins('dev')[0]

        assert recv == result

    def test_pause_connector(self):
        self.conn.RestartConnector('dev', 'logs-broker')

    def test_resume_connector(self):
        self.conn.ResumeConnector('dev', 'logs-broker')

    def test_restart_connector(self):
        self.conn.RestartConnector('dev', 'logs-broker')
    ...
    def test_create_connector(self):
        ...
        assert recv == self.conn.CreateConnector('dev', config)

    def test_set_connector_config(self):
